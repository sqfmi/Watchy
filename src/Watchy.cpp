...
void Watchy::handleButtonPress(){
	...
	if(guiState == kWatchFace_State){//enter menu state if coming from watch face
	...
	if(guiState == MAIN_MENU_STATE){//exit to watch face if already in menu
		...
		showMenu(menuIndex, false);//exit to menu if already in app
	}else if(guiState == kWatchFace_State){
		return;
	}
	}
	//Up Button
...
JSONVar& Watchy::getWeatherJSON(String cityID, String units, String lang, String url, String apiKey, uint8_t updateInterval)
{
	if(weatherIntervalCounter < 0) //-1 on first run, set to updateInterval
		weatherIntervalCounter = updateInterval;
